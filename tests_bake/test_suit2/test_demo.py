...
class Test_demo():
    ...
    def test_home(self, action: ElementActions):
        ...
        action.click(p.sale_home_page.search_box)

        #因为调用action的大部分公用方法是返回self，所以可以一条语句执行多次不同方法
        action.text(p.category_list_search_page.search_box, "口红")\
            .click(p.category_list_search_page.search_button)

        action.click(p.search_result_list_page.第一个商品项)
